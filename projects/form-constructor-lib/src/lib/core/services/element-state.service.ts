...
import { FormState, ElementPosition, FormElement } from '../../models/form-state.model';
...
export class ElementStateService {
  ...
  updateElementProperties(elementId: string, properties: Partial<FormElementProperties>): void {
    ...
    const newState: FormState = {
      ...
      elementProperties: {} as Record<string, FormElementProperties>,
      lastSaved: new Date()
    };
  ...
  addElement(element: EditorElement, rowIndex: number, colIndex: number): void {
    ...
    newState.elementProperties[element.id] = { ...element, type: element.type } as FormElementProperties;
  ...
  private statesEqual(state1: FormState, state2: FormState): boolean {
    ...
    for (const key of props1Keys) {
      ...
      for (const innerKey of props1KeysInner) {
        if ((props1 as any)[innerKey] !== (props2 as any)[innerKey]) return false;
      }
    }
    ...
    for (const key of formProps1Keys) {
      if ((state1.formProperties as any)[key] !== (state2.formProperties as any)[key]) return false;
    }
